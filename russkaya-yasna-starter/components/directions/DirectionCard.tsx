          ...
          <section className="pt-6">
            ...
            >
              {activities.map((activity) => (
                <motion.li key={activity} variants={itemVariants} className="flex gap-2">
              ...
              {results.map((testimonial) => {
                const content = (
                  <div className="flex h-full flex-col gap-3 rounded-xl border border-primary-100 bg-white p-4 shadow-sm">
                    <p className="text-sm italic text-text/70">“{testimonial.quote}”</p>
                ...
                return onTestimonialClick ? (
                  ...
                  <div key={testimonial.id} className="min-w-[240px] flex-1 md:min-w-0">
                    {content}
                  </div>
                );
              })}
              ...
              {resources.map((resource) => (
                ...
                >
                  <span className="text-accent">
                    {resource.icon ?? <ExternalLink className="h-4 w-4" aria-hidden="true" />}
                  </span>
